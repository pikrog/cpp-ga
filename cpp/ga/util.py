...
def pmx_insert(parent1, parent2, segment_begin, segment_end):
...
def abs_pmx_insert(parent1, parent2, segment_begin, segment_end):
    child = [None] * len(parent1)
...
def abs_pmx_crossover(parents, offspring_size, ga_instance: pygad.GA):
    return pmx_crossover(parents, offspring_size, ga_instance, abs_pmx_insert)


def pmx_crossover(parents, offspring_size, ga_instance: pygad.GA, inserter=pmx_insert):
    offspring = []
    index = 0
...
def random_select_elements(threshold, size, low=0.0, high=1.0):
